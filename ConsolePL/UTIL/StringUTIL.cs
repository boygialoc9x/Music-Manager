...
{ 
    public class StringUTil
    {
        ...
        private static bool CheckNoQuoteCharecterInString(string str)
        {
            bool Check = true;
            try
            {
        ...
        public static DateTime GetReleaseDate()
        {
            ...
            bool endloop = false;
            string title = "Please enter the release date that following\nthe format (Month/Day/Year): ";
            do
            {
                releaseDate = ReadBirthDate(title);
                if (releaseDate != DateTime.MinValue)
                {
                    if (releaseDate <= DateTime.Today) endloop = true;
                    else 
                    {
                        Console.WriteLine("Does this song release in the future ?");
                        Console.WriteLine("Do you want to retry?");
                        if (YNQuest.ask_YesOrNo()) endloop = false;
                        else 
                        {
                            releaseDate = DateTime.MinValue;
                            Console.WriteLine("Cancel enter the release date!");
                            endloop = true;
                        }
                    }
                } else endloop = true;
        ...
        public static DateTime GetTheTrueBithday()
        {
            ...
            bool endloop = false;
            string title = "Please enter the birthday that following\nthe format (Month/Day/Year): ";
            do
            {
                birthdate = ReadBirthDate(title);
        ...
        public static DateTime ReadBirthDate(string title)
        {
            DateTime birthdate;
            bool endloop = false;
            do
            {
                //Console.Clear();
                //UIHelper.MenuTitle("Entering Release Date");
                Console.Write(title);
                var tempBirthDay = Console.ReadLine();
                ...
                else
                {
                    ...
                    UIHelper.DrawStraightLine();
                    Console.WriteLine("Invalid Format. <Month/Day/Year>");
                    ...
                    if (YNQuest.ask_YesOrNo())
                    {
                        endloop = false;
                        Console.Clear();
                        UIHelper.MenuTitle(" Entering Valid Date Time");
                    }
                    else
                    {
                        Console.Clear();
